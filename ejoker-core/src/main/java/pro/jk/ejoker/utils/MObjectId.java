...
public class MObjectId implements Comparable<MObjectId>, Serializable {

    private static final long serialVersionUID = 3670079982654483072L;

    // static final Logger LOGGER = Loggers.getLogger("ObjectId");
    final static Logger LOGGER = LoggerFactory.getLogger(MObjectId.class);
     ...
     * @return the new id
     */
    public static MObjectId get() {
        return new MObjectId();
    }
     ...
     * @since 2.12.0
     */
    public static MObjectId createFromLegacyFormat(final int time, final int machine, final int inc) {
        return new MObjectId(time, machine, inc);
    }
     ...
     * @param date the date
     */
    public MObjectId(final Date date) {
     ...
     */
    public MObjectId(final Date date, final int counter) {
     ...
     */
    public MObjectId(final Date date, final int machineIdentifier, final short processIdentifier, final int counter) {
     ...
     * @throws IllegalArgumentException if the high order byte of machineIdentifier or counter is not zero
     */
    public MObjectId(final int timestamp, final int machineIdentifier, final short processIdentifier, final int counter) {
        this(timestamp, machineIdentifier, processIdentifier, counter, true);
    }

    private MObjectId(final int timestamp, final int machineIdentifier, final short processIdentifier, final int counter,
     ...
     */
    public MObjectId(final String hexString) {
     ...
     */
    public MObjectId(final byte[] bytes) {
     ...
     */
    MObjectId(final int timestamp, final int machineAndProcessIdentifier, final int counter) {
     ...
     * @since 3.4
     */
    public MObjectId(final ByteBuffer buffer) {
    ...
    public boolean equals(final Object o) {
        ...
        MObjectId objectId = (MObjectId) o;
    ...
    @Override
    public int compareTo(final MObjectId other) {
        if (other == null) {
    ...
    // ============================================= //
    // copy from org.bson.assertions.Assertions
    ...
    static {
    	Object object = null;
    	try {
			Class<?> MongoObjectIdClazz = Class.forName("org.bson.types.ObjectId");
			Field declaredField = MongoObjectIdClazz.getDeclaredField("NEXT_COUNTER");
			declaredField.setAccessible(true);
			object = declaredField.get(null);
		} catch (Exception e) {
		}
    	if(null != object)
    		NEXT_COUNTER = (AtomicInteger )object;
    	else
