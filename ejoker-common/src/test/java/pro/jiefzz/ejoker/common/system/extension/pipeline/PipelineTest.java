...
//	@Test
	...
	public Map<String, String> doCalculateOrStatistics(List<String> dataFromApi) {
		...
		dataFromApi.forEach(s -> res.put(s, StringHelper.fill("Hello {}, today is '{}', have a nice day~", s, new Date().toString())));
		return res;
	}
...
//	@Test
	...
	public List<String> doSth(Map<String, String> src) {
		...
		src.forEach((k, v) -> res.add(StringHelper.fill("Hello {}, u r {}, have a nice day~", k, v)));
		return res;
	}
...
//		for(int i=0; i<3; i++)
	...
	private String buildGoods(String name, String desc, AtomicInteger ai) {
		return StringHelper.fill("Curr index: {}, Today is {}, {} did great! and he/she is {}", ai.getAndIncrement(), new Date(), name, desc);
	}
	...
	public void testInExecutor4() throws InterruptedException {
		...
		decs.entrySet().parallelStream().forEach(e -> {
			fixedThreadPool.execute(new Pipeline<>(this::fixName, e.getKey())
					.addPipelineHook(new PipelineHook((cxt, ex, args) -> { System.err.println(StringHelper.fill("{} [{}]", ex.getMessage(), getCxtInfo(args)));ex.printStackTrace(); cxt.setPreventThrow();/**/}))
...
//		}
		return StringHelper.fill("OK! a: {}, b: {}, c: {}, d:{}, source: {}", a, b, c, d, source);
	}
