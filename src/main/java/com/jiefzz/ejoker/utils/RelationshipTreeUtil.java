...
public class RelationshipTreeUtil<ContainerKVP, ContainerVP> {
	...
	public ContainerKVP getTreeStructureMap(Object bean, RelationshipTreeUtilCallbackInterface<ContainerKVP, ContainerVP> eval) throws Exception{
		ObjectHolder convertKeyValueSet = new ObjectHolder();
		convertKeyValueSet.value = bean;
		return (ContainerKVP )eval.getOne(getTreeStructureMapInner(convertKeyValueSet, eval), "value");
	}
	private ContainerKVP getTreeStructureMapInner(Object bean, RelationshipTreeUtilCallbackInterface<ContainerKVP, ContainerVP> eval) throws Exception{
		...
		for ( ; clazz != Object.class; clazz = clazz.getSuperclass() ) {
			...
			for ( int i = 0; i<fieldArray.length;i++ ) {
				...
				if ( field.getType().isPrimitive() || ParameterizedTypeUtil.isDirectSerializableType(obj) )
					...
					eval.addToKeyValueSet(childrenObj, getTreeStructureMapInner(obj, eval), fieldName);
	...
	private ContainerKVP innerAssemblingKVP(RelationshipTreeUtilCallbackInterface<ContainerKVP, ContainerVP> eval, Object object) throws Exception {
		...
		for ( String key : keySet ) {
			...
			// 键值集中有空值传入，则写入空值，认为是调用者有意置空
			if (currentObject==null)
				eval.addToKeyValueSet(keyValueSet, null, key);
			else if ( ParameterizedTypeUtil.isDirectSerializableType(currentObject) )
				// 可直接序列化类型
				...
				eval.addToKeyValueSet(keyValueSet, getTreeStructureMapInner(currentObject, eval), key);
		}
		return keyValueSet;
	}
	...
	private ContainerVP innerAssemblingVP(RelationshipTreeUtilCallbackInterface<ContainerKVP, ContainerVP> eval, Object object) throws Exception {
		...
		for ( Object currentObject : objCollection ) {
			...
			if (currentObject==null) continue;
				...
				eval.addToValueSet(valueSet, getTreeStructureMapInner(currentObject, eval));
		}
		return valueSet;
	}
